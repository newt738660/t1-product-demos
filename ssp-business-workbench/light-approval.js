...
(() => {
  const KEY='ssp-light-approval-v1', LOCK=KEY+'-write';
  ...
  let actorId=sessionStorage.getItem(KEY+'-actor')||'S-1042', db, offset=0;
  const esc=v=>String(v??'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;').replaceAll("'",'&#39;');
  const person=id=>actors.find(x=>x.id===id), label=id=>person(id)?`${person(id).name} / ${id}`:'待领取';
  const allowed=()=>['商务','超级管理员'].includes(person(actorId)?.access);
  const now=()=>Date.now()+offset;
  function seed(){
    return {items:applications.map(a=>({...a,rev:0,owner:a.status==='待一审'?'':'S-1042',second:a.status==='待二审'||a.status==='待三审'?'S-1018':'',third:director,targetId:a.kind==='绑定申请'?(advertisers.find(x=>x.name===a.company)?.id||''):'',history:[],noticeAt:Date.now(),reminded:false})),events:[]};
  }
  ...
  load();
  if(!localStorage.getItem(KEY)){db.items.filter(a=>a.status==='待一审').forEach(a=>notice(a,'新'+a.kind+'｜待一审','请当班商务进入 SSP 领取后处理'));localStorage.setItem(KEY,JSON.stringify(db))}
  ...
  const can=a=>allowed()&&stage(a)>0&&handler(a)===actorId;
  ...
  async function mutate(id,expected,fn){
    ...
    try {return await navigator.locks.request(LOCK,()=>{load();const a=get(id);if(!allowed())throw Error('当前账号无审批页操作权限');if(!a||a.rev!==expected)throw Error('申请已更新，请查看最新状态后操作');fn(a);a.rev++;localStorage.setItem(KEY,JSON.stringify(db));closeModal();render();return true})}
    catch(e){closeModal();render();toast(e.message);return false}
  }
  ...
  render=function(){
    ...
    const control=`<details class="demo-tools"><summary>交互演示 · 切换人员 / 群消息预览</summary><div class="demo-controls"><label>演示账号 <select id="demoActor" class="select">${actors.map(x=>`<option value="${x.id}" ${x.id===actorId?'selected':''}>${x.name} / ${x.id}（${x.access}）</option>`).join('')}</select></label><button class="btn" id="advanceClock">模拟 10 分钟后</button><button class="btn" id="resetApproval">重置演示</button></div><p class="helper">仅演示交互，人员均为示例；本浏览器内共享数据。生产环境领取防重需由 SSP 服务端保障。不会向 TG 发消息；候选人精简名单尚待确认。</p><div id="noticePreview">${db.events.slice(0,5).map(e=>`<article class="notice-preview"><strong>${esc(e.title)}</strong><p>${esc(e.id)} · ${new Date(e.time).toLocaleTimeString('zh-CN')}</p><p>客户填写商务：${esc(e.contactSales)}（仅参考，不做映射）</p>${e.who?`<p>@${esc(person(e.who)?.name||e.who)}（示意提醒）</p>`:''}<p>${esc(e.action)}</p><button class="btn link" data-notice-open="${esc(e.id)}">在 SSP 查看申请</button></article>`).join('')}</div></details>`;
    ...
    document.querySelector('#resetApproval').onclick=()=>confirm('重置演示数据？','<p>仅重置本浏览器中的申请演示记录。</p>','确认重置',async()=>{await navigator.locks.request(LOCK,()=>{db=seed();db.items.filter(a=>a.status==='待一审').forEach(a=>notice(a,'新'+a.kind+'｜待一审','请当班商务进入 SSP 领取后处理'));localStorage.setItem(KEY,JSON.stringify(db))});state.view='review';state.selected=null;offset=0;closeModal();render()});
  ...
  reviewList=function(){const rows=db.items.filter(a=>a.kind===state.reviewType&&(state.reviewScope==='全部申请'||stage(a)));return `${managementHeader(state.reviewType)}<div class="warning-box">一审由当班商务主动领取。请先确认承接结果再联系客户；打开详情不等于领取。值班名单以商务群公告为准。</div><section class="card list-card"><div class="review-subtabs">${['待处理','全部申请'].map(k=>`<button class="review-subtab ${state.reviewScope===k?'active':''}" data-scope="${k}">${k}</button>`).join('')}</div><div class="table-wrap"><table><thead><tr><th>申请 ID</th><th>申请人 / 广告主</th><th>客户填写商务</th><th>审核阶段</th><th>当前处理人</th><th>操作</th></tr></thead><tbody>${rows.map(a=>`<tr><td>${a.id}</td><td>${esc(a.applicant)}<br>${esc(a.company)}</td><td>${esc(a.contactSales||'未填写')}</td><td>${statusHtml(a.status)}</td><td>${stage(a)?esc(label(handler(a))):'—'}</td><td><button class="btn link" data-open-light="${a.id}">${can(a)?'继续处理':stage(a)===1&&!a.owner?'查看并领取':'查看'}</button></td></tr>`).join('')}</tbody></table></div></section>`};
  ...
  reviewDetail=function(a){
    ...
    const options=actors.filter(p=>p.access==='商务'&&p.id!==a.owner&&p.id!==director);
    const routing=n===1?`<div class="field"><label>二审接收人 *</label><select id="secondReviewer" class="select"><option value="">请选择应接收本申请的二审人员</option>${options.map(p=>`<option value="${p.id}" ${state.secondId===p.id?'selected':''}>${p.name} / ${p.id} · ${p.dept}</option>`).join('')}</select><p class="helper">请选择业务上应接收的组长。系统不判断上下级关系，请核对后提交。</p></div>`:n===2?`<div class="selected-advertiser"><strong>三审接收人（固定）</strong><span>${esc(label(a.third))}</span><p>无需选择，不可修改。</p></div>`:'';
    let panel=!n?`<h3>${a.status}</h3><p>当前没有可提交的审批任务。</p>`:!mine?`<h3>${n===1&&!a.owner?'待领取':`${['','一审','二审','三审'][n]}处理中`}</h3><p>${n===1&&!a.owner?'请当班商务主动承接；领取成功后再联系客户。':`当前由 ${esc(label(handler(a)))} 处理，其他人员只读查看。`}</p>${n===1&&!a.owner?'<button class="btn primary" id="claimTask">领取并开始一审</button>':''}`:`<h3>${['','一审','二审','三审'][n]}</h3>${n===1?(opening?openingReviewFields(a):searchPicker()):''}${routing}<div class="decision-actions"><button class="btn primary" data-decision="approve">${n===1?'一审通过并提交二审':n===2?'二审通过并提交三审':opening?'三审通过并创建广告主':'三审通过并确认绑定'}</button><button class="btn" data-decision="supplement">退回补充资料</button><button class="btn" data-decision="reject">驳回申请</button>${n===1?'<button class="btn link" id="releaseTask">释放领取</button>':n===2?'<button class="btn link" id="returnWrongReceiver">非本人应审，退回一审更正</button>':''}</div>`;
    return `<button class="back-link" data-action="back">← 返回${a.kind}</button><div class="page-head"><div class="page-title"><h1>${mine?'处理':'查看'}${a.kind}</h1><p>${a.id} · ${a.status} · 当前处理人：${n?esc(label(handler(a))):'—'}</p></div></div>${auditFlow(n||4)}<div class="review-detail"><div class="card tab-content"><div class="section"><div class="section-title">T1 提交信息</div><div class="info-grid">${info('申请人',a.applicant)}${info('联系方式',a.contactApp+' · '+a.contact)}${info('广告主名称',a.company)}${info('产品名称',a.product)}${info('网址链接',a.site)}${info('客户填写商务（仅参考）',a.contactSales)}</div></div><div class="section"><div class="section-title">本次审批流向</div><p>一审：${esc(label(a.owner))} → 二审：${a.second?esc(label(a.second)):'一审提交时选择'} → 三审：${esc(label(a.third))}（固定）</p><p>最终开户名称：${esc(a.finalName||'一审确认后展示')}</p><p>归属销售：${esc(a.sales||'—')} · 部门：${esc(a.salesDept||'—')}</p><p>绑定目标：${esc(a.targetId||'—')}</p>${a.resultId?`<p>执行结果：${a.resultId} · ${esc(a.resultText)}</p>`:''}</div><div class="section"><div class="section-title">处理记录</div>${history||'<p>暂无本次操作记录</p>'}${a.reason?`<div class="warning-box">${esc(a.reason)}</div>`:''}${withdraw}</div><div class="section"><div class="section-title">审批说明</div><p>审批在 SSP 内完成。一审、二审不执行开户或绑定，三审通过后才执行。客户填写商务不用于自动分派。人员选择不校验上下级关系。</p></div></div><aside class="card review-side decision-card">${panel}</aside></div>`;
  };
  bind=function(){oldBind();document.querySelectorAll('[data-open-light]').forEach(b=>b.onclick=()=>{hydrate(get(b.dataset.openLight));state.view='detail';render();scrollTo(0,0)});
    ...
    document.querySelector('#claimTask')?.addEventListener('click',async()=>{const ok=await mutate(a.id,a.rev,x=>{if(x.status!=='待一审'||x.owner)throw Error('该申请已被领取');x.owner=actorId;log(x,'领取一审');notice(x,'一审已承接',`${label(actorId)} 已领取，其他商务无需重复跟进`)});if(ok){hydrate(get(a.id));render();toast('领取成功，可以开始一审')}});
    document.querySelector('#releaseTask')?.addEventListener('click',()=>confirm('释放此申请？','<p>申请将回到待领取，本次未提交的填写不保留。请先在群内说明交接，避免重复联系客户。</p>','确认释放',()=>mutate(a.id,a.rev,x=>{if(!can(x)||x.status!=='待一审')throw Error('当前不能释放');log(x,'释放一审领取');x.owner='';notice(x,'一审待重新承接','原承接人已释放，请商务人工协调接手')})));
  ...
  decisionModal=function(decision,a){
    ...
    const selectedSecond=state.secondId, selectedSales=state.selectedSales, selectedTarget=state.selectedAdvertiser, name=state.advertiserName?.trim();
    if(decision==='approve'&&n===1){if(a.kind==='开户申请'&&(!selectedSales||!name||name.length<2||name.length>100)){toast('请填写2～100字广告主名称并选择归属销售');return}if(a.kind==='绑定申请'&&!selectedTarget){toast('请选择目标广告主');return}if(!selectedSecond||selectedSecond===a.owner||selectedSecond===director||person(selectedSecond)?.access!=='商务'){toast('请选择有效的二审接收人');return}}
    const titles={approve:n===1?'确认提交二审':n===2?'确认提交固定总监三审':'确认三审通过',supplement:'退回补充资料',reject:'驳回申请'};
    const route=n===1?`<div class="selected-advertiser"><strong>二审接收人：${esc(label(selectedSecond))}</strong><span>${esc(person(selectedSecond)?.dept)}</span></div><label class="confirm-check"><input id="confirmReviewer" type="checkbox"> 我已核对，此人为本申请应提交的二审负责人</label>`:n===2?`<p>三审固定提交给：<strong>${esc(label(a.third))}</strong></p>`:`<p>${a.kind==='开户申请'?'创建 SSP 广告主并绑定 T1 用户':'将 T1 用户绑定至 '+esc(a.targetId)}。请确认前两级结论。</p>`;
    confirm(titles[decision],decision==='approve'?route:'<label>具体原因（5～500字） *</label><textarea id="decisionReason" class="textarea"></textarea>',titles[decision],()=>{
      ...
      return mutate(a.id,rev,x=>{if(!can(x)||stage(x)!==n)throw Error('申请处理人或状态已变化');
        ...
        else if(n===2){if(x.third!==director||!person(director))throw Error('固定三审负责人异常，请联系 SSP 维护');x.status='待三审';log(x,'二审通过，提交固定总监');notice(x,'二审通过｜待三审','请完成最终审核',director)}
  ...
  async function sendReminders(){if(!navigator.locks)return;await navigator.locks.request(LOCK,()=>{load();db.items.forEach(a=>{if(a.status==='待一审'&&!a.owner&&!a.reminded&&now()-a.noticeAt>=600000){a.reminded=true;notice(a,'10分钟无人领取提醒','请当班主责查看，缺席时由备援按群内约定接手。')} });localStorage.setItem(KEY,JSON.stringify(db))});render()}
  addEventListener('storage',e=>{if(e.key===KEY){closeModal();load();if(state.selected)hydrate(get(state.selected.id));render()}});
