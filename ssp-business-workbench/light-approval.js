...
(() => {
  const KEY='ssp-direct-review-v3', LOCK=KEY+'-write';
  ...
  decisionModal=function(decision,a){
    if(!['approve','reject'].includes(decision)||!can(a)){toast('当前申请不可由您处理');return}const n=stage(a), rev=a.rev;
    ...
    const titles={approve:n===1?'确认提交二审':n===2?'确认提交三审':'确认三审通过',reject:'驳回申请'};
    ...
    confirm(titles[decision],decision==='approve'?route:'<p>驳回后本次申请结束，用户可查看原因、修改后重新申请。</p><label>驳回原因（5～500字，将向申请人展示） *</label><textarea id="decisionReason" class="textarea" placeholder="请说明哪里不符合要求，以及需要如何修改"></textarea>',titles[decision],()=>{
      ...
      return mutate(a.id,rev,x=>{if(!can(x)||stage(x)!==n)throw Error('申请处理人或状态已变化');
        ...
        if(decision!=='approve'){x.status='已驳回';x.reason=reason;x.rejectedAt=new Date(now()).toISOString();log(x,titles[decision]+'：'+reason);notice(x,x.status,reason);return}
