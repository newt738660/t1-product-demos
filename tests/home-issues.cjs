...
(async()=>{
 ...
 const geometry=await page.locator('#homeIssues').evaluate(el=>({height:el.getBoundingClientRect().height,x:[...el.querySelectorAll('.home-issue-actions')].map(a=>a.getBoundingClientRect().left)}));
 assert.ok(geometry.height<350,`Compact card too tall: ${geometry.height}`);assert.ok(Math.max(...geometry.x)-Math.min(...geometry.x)<1,'Actions must align');
 const first=page.locator('.home-issue-row').first();await first.getByRole('button',{name:'详情',exact:true}).click();assert.equal(await first.locator('.home-issue-detail').isVisible(),true);await first.getByRole('button',{name:'收起',exact:true}).click();assert.equal(await first.locator('.home-issue-detail').isVisible(),false);
 console.log('Compact height:',geometry.height,'px; action columns aligned; inline details verified');
 ...
 await page.getByRole('button',{name:'已忽略（1）',exact:true}).click();await risk.getByRole('button',{name:'恢复',exact:true}).click();
