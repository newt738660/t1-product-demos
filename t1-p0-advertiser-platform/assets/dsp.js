...
const App = {
  ...
  view_groupdetail(){
    ...
    ${!isRtb?`<details class="card group-setting-summary group-settings-collapsible">
      <summary class="cpd-settings-summary">
        <div class="group-summary-head"><div><b>投放设置</b><span class="badge ${groupIssue.cls}">${groupIssue.label}</span>${groupIssue.key!=='ready'?`<span class="group-issue-reason">${deliveryText}</span>`:''}<span class="cell-sub">由运营管理，仅供查看</span></div><span class="settings-toggle-label">展开详情</span></div>
  ...
  openCpdCreativeEdit(id){
    ...
    this.editFlow={type:'creative',id,cpd:true,returnPage:this.curGroup?'groupdetail':'campdetail'};
