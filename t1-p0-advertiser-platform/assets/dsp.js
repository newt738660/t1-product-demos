...
function creativeThumb(a){
  const preview=DB.assetFiles.find(f=>f.id===a.assetId)?.preview;if(preview)return `<img class="creative-thumb" src="${preview}" alt="创意素材预览">`;
  const byCampaign={'C-1002':'automotive.jpg','C-1003':'gaming.jpg','C-1004':'shopping.jpg','C-1005':'gaming.jpg'};
  const byFormat={splashimg:'automotive.jpg',reward:'gaming.jpg',native:'shopping.jpg',feed:'shopping.jpg',video:'brand.jpg'};
  const f=DB.assetFiles.find(x=>x.id===a.assetId);const file=(f?(f.dim==='1080×1920'?'automotive.jpg':'shopping.jpg'):null)||byCampaign[a.camp]||byFormat[a.fmt]||'brand.jpg';
  return `<img class="creative-thumb" src="assets/creative-images/${file}" alt="${a.name}创意图片">`;
}
...
const DB = {
  ...
  campaigns: [
    ...
    {id:'C-1007', scenario:'state-case', name:'数据同步异常｜待处理', alias:'同步异常', mode:'cpd', placement:'T1 News App · 文章页横幅', period:'2026-08-16 至 2026-08-30', duration:'fixed', start:'2026-08-16', end:'2026-08-30', fmt:'mbanner', model:'CPD', status:'active', syncStatus:'failed', issueReason:'SSP 最近一次同步失败，当前计划状态和数据可能不是最新结果。系统将在 10 分钟后自动重试；如持续失败，请联系运营。', imps:0, clicks:0, operator:'Linda Zhao', order:'CPD-202608-024'},
  ...
  creatives: [
    ...
    {id:'CR-3007',scenario:'state-case',name:'游戏预约开屏',group:'游戏开屏｜预约期',groupId:'G-2005',fmt:'splashimg',kind:'image',size:'1080×1920',camp:'C-1005',headline:'预约游戏新版本',description:'当前线上生效内容',landing:'https://www.t1t1.com/game',imps:228000,clicks:1368,status:'active',version:1,created:'2026-08-15',changeStatus:'review',pendingVersion:{version:2,name:'游戏预约新开屏',headline:'立即预约新版本',description:'全新角色预约即得好礼',jumpType:'external',landing:'https://www.t1t1.com/game-new',status:'review',submittedAt:'2026-08-17T10:20:00Z'}},
...
const App = {
  ...
  after_plans(){
    ...
    const search=document.getElementById('unifiedSearch');if(search)search.placeholder=this.unifiedType==='rtb'?'搜索计划名称 / ID':'搜索计划名称 / ID / 订单编号';
    this.renderUnifiedPlans();
  },
  setUnifiedType(btn,type){ document.querySelectorAll('#unifiedTypeTabs button').forEach(x=>x.classList.remove('active'));btn.classList.add('active');this.unifiedType=type;DB.uiState.planListView=type;this.save();this.pages.unifiedPlan=1;this.unifiedSelected=new Set();const search=document.getElementById('unifiedSearch');if(search)search.placeholder=type==='rtb'?'搜索计划名称 / ID':'搜索计划名称 / ID / 订单编号';this.renderUnifiedPlans();const wrap=document.querySelector('.plan-list-card .table-wrap');if(wrap)wrap.scrollLeft=0; },
  ...
  unifiedPlanIssue(cp,creatives){
    ...
    if(creatives.some(a=>a.status==='review'||a.pendingVersion?.status==='review')&&!creatives.some(a=>a.status==='active'))return {key:'review',label:'创意审核中',cls:'amber'};
    if(creatives.length&&creatives.every(a=>a.status==='rejected'))return {key:'rejected',label:'创意已驳回',cls:'red'};
    if(!creatives.some(a=>a.status==='active'))return {key:'no-creative',label:'暂无可投创意',cls:'amber'};
    return {key:'ready',label:cp.mode==='cpd'?'运营代投中':'正常投放',cls:'green'};
  },
  groupDeliveryIssue(group,cp,creatives){
    const planIssue=this.unifiedPlanIssue(cp,DB.creatives.filter(a=>a.camp===cp.id));
    if(['error','paused','ended','upcoming'].includes(planIssue.key))return planIssue;
    if(group.status==='paused')return {key:'group-paused',label:'广告组已暂停',cls:'gray'};
    if(group.status==='ended')return {key:'group-ended',label:'广告组已结束',cls:'gray'};
    if(creatives.some(a=>a.status==='review'||a.pendingVersion?.status==='review')&&!creatives.some(a=>a.status==='active'))return {key:'review',label:'创意审核中',cls:'amber'};
    if(creatives.length&&creatives.every(a=>a.status==='rejected'))return {key:'rejected',label:'创意已驳回',cls:'red'};
    if(!creatives.some(a=>a.status==='active'))return {key:'no-creative',label:'暂无可投创意',cls:'amber'};
    return {key:'ready',label:cp.mode==='cpd'?'运营代投中':'正常投放',cls:'green'};
  },
  creativeDeliveryIssue(creative,group,cp){
    const parent=this.groupDeliveryIssue(group||{},cp||{},[creative]);
    if(['error','paused','ended','upcoming','group-paused','group-ended'].includes(parent.key))return parent;
    if(creative.status==='review')return {key:'review',label:'审核中，暂未投放',cls:'amber'};
    if(creative.status==='rejected')return {key:'rejected',label:'审核未通过',cls:'red'};
    if(creative.status==='paused')return {key:'creative-paused',label:'创意已暂停',cls:'gray'};
    if(creative.status==='ended')return {key:'creative-ended',label:'创意已结束',cls:'gray'};
    return {key:'ready',label:'当前版本投放中',cls:'green'};
  },
  deliveryIssueDescription(issue,cp){
    const descriptions={ready:cp.mode==='cpd'?'计划当前具备投放条件，由运营正常代投。':'计划当前具备投放条件，正在正常投放。',upcoming:'尚未到计划开始时间。',paused:'计划已暂停，因此下属广告组和创意均不能投放。',ended:'计划已结束，因此下属广告组和创意均不能继续投放。',review:'当前没有可投的新版本，创意仍在审核中。',rejected:'当前没有可投创意，提交内容未通过审核。','no-creative':'计划和广告组已开启，但当前没有满足投放条件的创意。',error:cp.issueReason||'当前存在异常，暂时无法正常投放。'};
    return descriptions[issue.key]||'请查看下属广告组和创意的投放情况。';
  },
  showDeliveryIssue(level,name,issue,cp){
    this.modal(`<div class="modal-head"><div><h3>投放情况说明</h3><p>${level}：${name}</p></div><div class="spacer"></div><button class="icon-btn" onclick="App.closeModal()">${svg(I.x)}</button></div><div class="modal-body"><div class="notice ${issue.cls==='red'?'warning':'info'}"><span class="badge ${issue.cls}">${issue.label}</span><div style="margin-top:10px;line-height:1.7">${this.deliveryIssueDescription(issue,cp)}</div></div>${cp.issueReason?`<div class="card card-pad" style="margin-top:14px"><b>异常详情</b><div style="margin-top:8px;line-height:1.7">${cp.issueReason}</div></div>`:''}</div><div class="modal-foot"><button class="btn btn-primary" onclick="App.closeModal()">我知道了</button></div>`);
  },
  renderUnifiedPlans(){
    ...
    const heads=[['选择','select-col'],...(showType?[['投放类型','plan-type-col']]:[]),['广告计划','plan-name-col'],['状态','status-col'],['投放情况','issue-col'],['预算','money-col'],['花费','money-col'],['剩余预算','money-col wide'],['曝光','num metric-col'],['点击','num metric-col'],['CTR','num metric-col'],['CPC','num metric-col'],['CPM','num metric-col'],['开始日期','date-col'],['结束日期','date-col'],...(showType?[['订单编号','order-col'],['代投运营','owner-col']]:[]),['操作','act']];
  ...
  applyPlanStatus(ids,status){let changed=0;ids.forEach(id=>{const cp=DB.campaigns.find(c=>c.id===id);if(cp?.mode==='rtb'&&cp.status!=='archived'&&cp.status!==status){cp.status=status;changed++;}});this.save();this.closeModal();this.unifiedSelected=new Set();if(this.cur==='campdetail')this.go('campdetail');else this.renderUnifiedPlans();this.toast(`${changed} 条广告计划已${status==='paused'?'暂停':'恢复'}`);},
  ...
  togglePlan(id, e){
    ...
    if(!['active','paused'].includes(cp.status))return;
    this.confirmPlanStatus(id,cp.status==='active'?'paused':'active');
  },
  ...
  view_campdetail(){
    ...
    <div class="page-head campaign-detail-head">
      <div><div class="detail-title-line"><h1>${cp.name}</h1>${this.detailEnabledBadge(cp.status)}</div><p>${cp.id} · ${cp.mode==='cpd'?'CPD 运营代投':'RTB 自助投放'} · ${cp.alias||'未设置客户备注'}</p></div>
      <div class="spacer"></div>
    ...
    <div class="card group-setting-summary" aria-label="广告计划概览">
      <div class="group-summary-head">
        <div><b>投放概览</b><span class="badge ${planIssue.cls}">${planIssue.label}</span>${planIssue.key!=='ready'?`<span class="group-issue-reason">${planIssueText}</span>`:''}</div>
      ...
      <dl class="group-setting-grid plan-setting-grid">
        ...
        ${cp.mode==='rtb'?`<div><dt>${cp.duration==='ongoing'?'每日预算':'总预算'}</dt><dd>${fmtMoney(budget||0)}</dd></div><div><dt>每日上限</dt><dd>${cp.dailyCap?fmtMoney(cp.dailyCap):'未单独设置'}</dd></div>`:''}
      </dl>
      <dl class="group-setting-grid detail-performance" aria-label="累计投放数据"><div><dt>累计曝光</dt><dd>${fmtNum(cp.imps||0)}</dd></div><div><dt>累计点击</dt><dd>${fmtNum(cp.clicks||0)}</dd></div><div><dt>CTR</dt><dd>${cp.imps?((cp.clicks||0)/cp.imps*100).toFixed(2)+'%':'—'}</dd></div>${cp.mode==='rtb'?`<div><dt>累计花费</dt><dd>${fmtMoney(spend)}</dd></div>`:''}</dl>
    </div>
    ...
    <div class="card group-list-card"><div class="card-head"><div><b>广告组</b><div class="cell-sub">共 <span id="visibleGroupCount">${groups.length}</span> 个广告组；点击名称进入广告组详情</div></div><div class="spacer"></div>${groups.length?`<input class="input group-search" id="campaignGroupSearch" placeholder="搜索广告组名称 / ID" oninput="App.filterCampaignGroups()"><select class="select group-status-filter" id="campaignGroupStatus" style="box-sizing:border-box;flex:0 0 200px;width:200px;min-width:200px;max-width:200px;padding-left:16px;padding-right:44px" onchange="App.filterCampaignGroups()"><option value="">全部状态</option><option value="active">已启用</option><option value="draft">草稿</option><option value="review">审核中</option><option value="paused">已暂停</option><option value="needs-creative">待创建创意</option></select>`:''}</div>
      <div class="table-wrap"><table class="campaign-group-table ${cp.mode==='cpd'?'cpd-readonly-table':''}"><thead><tr><th>广告组</th><th>状态</th><th>投放情况</th><th>排期</th>${cp.mode==='cpd'?'<th>广告形式</th><th>投放位置</th>':'<th>预算</th><th>竞价方式</th><th>出价</th>'}<th class="num">创意</th><th class="num">曝光</th><th class="num">点击</th><th class="num">CTR</th>${cp.mode==='rtb'?'<th class="act">操作</th>':''}</tr></thead><tbody id="campaignGroupRows">
      ${groups.map(g=>{const ga=ads.filter(a=>a.groupId===g.id||(!a.groupId&&a.camp===g.camp&&a.group===g.name)),gi=ga.reduce((s,a)=>s+(a.imps||0),0),gc=ga.reduce((s,a)=>s+(a.clicks||0),0),gctr=gi?(gc/gi*100).toFixed(2)+'%':'—',needsCreative=ga.length===0,groupIssue=this.groupDeliveryIssue(g,cp,ga),canToggle=['active','paused'].includes(g.status),toggleLabel=g.status==='paused'?'恢复':'暂停',actions=`<div class="plan-row-actions"><button class="plan-row-action" onclick="App.openGroupEdit('${g.id}','campdetail')">编辑</button>${canToggle?`<button class="plan-row-action" onclick="App.confirmGroupStatus('${g.id}','campdetail')">${toggleLabel}</button>`:''}</div>`;return `<tr data-group-row data-search="${(g.name+' '+g.id).toLowerCase()}" data-status="${needsCreative?'needs-creative':g.status}"><td><button class="link-btn" onclick="App.openGroupDetail('${g.id}','${g.name.replaceAll("'","&#39;")}')"><b>${g.name}</b></button><div class="cell-sub">${g.id}</div></td><td>${this.detailEnabledBadge(g.status)}</td><td><span class="badge ${groupIssue.cls}">${groupIssue.label}</span></td><td>${g.start||cp.start||'—'}<div class="cell-sub">${g.end?'至 '+g.end:'长期投放'}</div></td>${cp.mode==='cpd'?`<td>${FMT[g.format]?.name||'—'}</td><td>${g.inventory||cp.placement||'—'}</td>`:`<td>${fmtMoney(g.budget||0)}</td><td>${g.bidType||'CPM'}</td><td>${fmtMoney(g.bid||0)}</td>`}<td class="num">${needsCreative?(cp.mode==='rtb'?`<button class="text-link" onclick="App.openNewCreativeForGroup('${g.id}')">创建创意</button>`:'<span class="badge amber">0 · 待补充</span>'):ga.length}</td><td class="num">${fmtK(gi)}</td><td class="num">${fmtK(gc)}</td><td class="num">${gctr}</td>${cp.mode==='rtb'?`<td class="act">${actions}</td>`:''}</tr>`}).join('')||`<tr><td colspan="${cp.mode==='rtb'?12:10}"><div class="empty">${svg(I.group||I.camp)}<div>当前广告计划还没有广告组</div>${cp.mode==='rtb'?`<button class="btn btn-primary btn-sm" onclick="App.startNewGroup('${cp.id}')">创建第一个广告组</button>`:''}</div></td></tr>`}
      </tbody></table></div></div>`;
  },
  ...
  switchCampaignTab(btn, tab){
    ...
    if(tab==='groups') panel.innerHTML=`<div class="card"><div class="card-head"><div><b>广告组</b><div class="cell-sub">进入广告组可查看当前创意或继续新建创意</div></div>${cp.mode==='rtb'?`<div class="spacer"></div><button class="btn btn-primary btn-sm" onclick="App.startNewGroup('${cp.id}')">${svg(I.plus)}新建广告组</button>`:''}</div><div class="table-wrap"><table><thead><tr><th>广告组</th><th>广告位与规格</th><th>状态</th><th class="num">创意</th><th class="num">曝光</th><th class="num">点击</th><th class="num">CTR</th><th class="act">操作</th></tr></thead><tbody>${groups.map(g=>{const ga=ads.filter(a=>(a.group||'默认广告组')===g),gi=ga.reduce((s,a)=>s+a.imps,0),gc=ga.reduce((s,a)=>s+a.clicks,0),gctr=gi?(gc/gi*100).toFixed(2)+'%':'—',obj=groupObjects.find(x=>x.name===g);return `<tr><td><div class="cell-main" style="color:var(--accent);cursor:pointer" onclick="App.openGroupDetail('${obj?.id||''}','${g.replaceAll("'","&#39;")}')">${g}</div><div class="cell-sub">${obj?.id||'与 SSP 广告组同步'}</div></td><td>${obj?.inventory||cp.placement||'—'}<div class="cell-sub">${ga[0]?.size||FMT[obj?.format]?.name||'待添加创意'}</div></td><td>${this.statusBadge(obj?.status||'active')}</td><td class="num">${ga.length}</td><td class="num">${fmtK(gi)}</td><td class="num">${fmtK(gc)}</td><td class="num">${gctr}</td><td class="act"><button class="btn btn-ghost btn-sm" onclick="App.openGroupDetail('${obj?.id||''}','${g.replaceAll("'","&#39;")}')">进入广告组</button></td></tr>`}).join('')||'<tr><td colspan="8"><div class="empty">暂无广告组</div></td></tr>'}</tbody></table></div></div>`;
  ...
  view_groupdetail(){
    ...
    const cp=DB.campaigns.find(c=>c.id===group.camp)||{},ads=DB.creatives.filter(a=>a.camp===cp.id&&(a.groupId===group.id||(!a.groupId&&a.group===group.name)));
    ...
    <div class="page-head"><div><div class="detail-title-line"><h1>${group.name}</h1>${this.detailEnabledBadge(group.status)}</div><p>${group.id||'SSP 同步广告组'} · 所属计划：${cp.name||'—'}</p></div><div class="spacer"></div>${isRtb?`<button class="btn btn-ghost" onclick="App.openGroupEdit('${group.id}')">${svg(I.edit)}编辑广告组</button>${groupCanToggle?`<button class="btn btn-ghost" onclick="App.confirmGroupStatus('${group.id}')">${svg(group.status==='paused'?I.play:I.pause)}${groupToggleLabel}</button>`:''}<button class="btn btn-ghost" onclick="App.confirmDeleteDeliveryObject('group','${group.id}')">${svg(I.trash)}删除</button>`:''}</div>
    ${isRtb?`<div class="card group-setting-summary group-settings-collapsible">
      <div class="group-summary-head">
        ...
        <button type="button" class="settings-toggle-label" aria-expanded="false" onclick="App.toggleCpdGroupSettings(event,this)">展开详情</button>
      </div>
      <dl class="group-setting-grid">
        ...
        <div><dt>预算</dt><dd>${fmtMoney(group.budget||0)}</dd></div>
      ...
      <dl class="group-setting-grid detail-performance" aria-label="累计投放数据"><div><dt>累计花费</dt><dd>${fmtMoney(groupSpend)}</dd></div><div><dt>累计曝光</dt><dd>${fmtNum(groupImps)}</dd></div><div><dt>累计点击</dt><dd>${fmtNum(ads.reduce((n,a)=>n+(a.clicks||0),0))}</dd></div></dl>
      <div class="cpd-group-sections" hidden>${this.detailTargetFields(group,cp)}</div>
    </div>`:''}
    ${!isRtb?`<div class="card group-setting-summary group-settings-collapsible">
      ...
      <div class="cpd-group-sections" hidden>
        ...
        ${this.detailTargetFields(group,cp)}
      </div>
    </div>`:''}
    <div class="card creative-priority-card"><div class="card-head"><div><b>广告创意</b></div>${isRtb?`<div class="spacer"></div><button class="btn btn-primary btn-sm" onclick="App.openNewCreativeForGroup('${group.id}')">${svg(I.plus)}新建创意</button>`:''}</div><div class="table-wrap"><table><thead><tr><th>广告创意</th><th>素材规格</th><th>审核状态</th><th>投放状态</th><th class="num">曝光</th><th class="num">点击</th><th class="num">CTR</th><th>更新时间</th><th>操作</th></tr></thead><tbody>${ads.map(a=>{const ctr=a.imps?((a.clicks||0)/a.imps*100).toFixed(2)+'%':'—',creativeIssue=this.creativeDeliveryIssue(a,group,cp),reviewState=a.pendingVersion?'<span class="badge green">当前版本已通过</span><div class="cell-sub" style="margin-top:4px">'+(a.pendingVersion.status==='rejected'?'新版本被驳回':'新版本审核中')+'</div>':this.detailReviewBadge(a),canToggle=isRtb&&['active','paused'].includes(a.status),toggleLabel=a.status==='paused'?'恢复':'暂停',updated=a.pendingVersion?.submittedAt?new Date(a.pendingVersion.submittedAt).toLocaleDateString('zh-CN'):a.updated||a.created||'—';return `<tr><td><div class="cell-flex">${creativeThumb(a)}<div><button class="link-btn" onclick="App.openCreativePreview('${a.id}')"><b>${a.name}</b></button><div class="cell-sub">${a.id} · V${a.version||1} · ${a.headline||'未填写标题'}</div></div></div></td><td>${a.size||'—'}</td><td>${reviewState}</td><td><span class="badge ${creativeIssue.cls}">${creativeIssue.label}</span></td><td class="num">${fmtK(a.imps||0)}</td><td class="num">${fmtK(a.clicks||0)}</td><td class="num">${ctr}</td><td>${updated}</td><td><div class="cell-flex"><button class="plan-row-action" onclick="App.openCreative('${a.id}')">编辑</button>${canToggle?`<button class="plan-row-action" onclick="App.confirmCreativeStatus('${a.id}')">${toggleLabel}</button>`:''}<button class="plan-row-action" onclick="App.confirmDeleteDeliveryObject('creative','${a.id}')">删除</button></div></td></tr>`}).join('')||`<tr><td colspan="9"><div class="empty">${svg(I.creative)}<div>当前广告组还没有广告创意</div></div></td></tr>`}</tbody></table></div></div>`;
  },
  openCreativePreview(id){
    const a=DB.creatives.find(x=>x.id===id);if(!a)return;const cp=DB.campaigns.find(c=>c.id===a.camp)||{},group=(DB.adGroups||[]).find(g=>g.id===a.groupId)||{},asset=DB.assetFiles.find(f=>f.id===a.assetId)||{},issue=this.creativeDeliveryIssue(a,group,cp),ctr=a.imps?((a.clicks||0)/a.imps*100).toFixed(2)+'%':'—';
    this.modal(`<div class="modal-head"><div><h3>${a.name}</h3><p>${a.id} · 当前版本 V${a.version||1}</p></div><div class="spacer"></div><span class="badge ${issue.cls}">${issue.label}</span><button class="icon-btn" onclick="App.closeModal()">${svg(I.x)}</button></div><div class="modal-body">${a.pendingVersion?`<div class="notice info" style="margin-bottom:16px"><b>${a.pendingVersion.status==='rejected'?'新版本被驳回':'新版本审核中'}</b><div style="margin-top:5px">当前展示的是已通过的 V${a.version||1}；实际投放受启停、排期和预算约束。新内容审核通过后才会替换。</div></div>`:''}<div class="creative-preview-modal"><div class="creative-preview-large">${creativeThumb(a)}<span>${asset.name||a.size||'当前素材'}</span></div><div><dl class="cpd-readonly-grid"><div class="wide"><dt>所属广告计划</dt><dd>${cp.name||'—'}</dd></div><div class="wide"><dt>所属广告组</dt><dd>${group.name||a.group||'—'}</dd></div><div><dt>素材规格</dt><dd>${a.size||asset.dim||'—'}</dd></div><div><dt>曝光</dt><dd>${fmtK(a.imps||0)}</dd></div><div><dt>点击</dt><dd>${fmtK(a.clicks||0)}</dd></div><div><dt>CTR</dt><dd>${ctr}</dd></div><div><dt>广告形式</dt><dd>${({splashimg:'开屏',feed:'信息流',bbanner:'横幅',rect:'矩形',micon:'图标',text:'文字'})[a.fmt]||FMT[a.fmt]?.name||'—'}</dd></div><div class="wide"><dt>标题</dt><dd>${a.headline||'—'}</dd></div><div class="wide"><dt>描述</dt><dd>${a.description||'—'}</dd></div><div class="wide"><dt>CTA文案</dt><dd>${a.cta||'—'}</dd></div><div><dt>跳转类型</dt><dd>外部跳转</dd></div><div class="wide"><dt>目标链接</dt><dd class="mono">${a.landing||'—'}</dd></div></dl></div></div></div><div class="modal-foot"><div class="spacer"></div><button class="btn btn-primary" onclick="App.closeModal()">关闭</button></div>`,true);
  },
  confirmGroupStatus(id,returnPage='groupdetail'){
    ...
    this.modal(`<div class="modal-head"><div><h3>${pause?'暂停':'恢复'}广告组</h3><p>${group.name} · ${group.id}</p></div></div><div class="modal-body">${content}</div><div class="modal-foot"><button class="btn btn-ghost" onclick="App.closeModal()">取消</button><button class="btn btn-primary" onclick="App.closeModal();App.toggleGroupStatus('${id}','${returnPage}')">确认${pause?'暂停':'恢复'}</button></div>`,true);
  },
  ...
  confirmCreativeStatus(id,returnPage='groupdetail'){
    ...
    this.modal(`<div class="modal-head"><div><h3>${pause?'暂停':'恢复'}广告创意</h3><p>${creative.name} · ${creative.id}</p></div></div><div class="modal-body">${content}</div><div class="modal-foot"><button class="btn btn-ghost" onclick="App.closeModal()">取消</button><button class="btn btn-primary" onclick="App.closeModal();App.toggleCreativeStatus('${id}','${returnPage}')">确认${pause?'暂停':'恢复'}</button></div>`,true);
  },
  toggleCreativeStatus(id,returnPage='groupdetail'){
    const creative=DB.creatives.find(a=>a.id===id);if(!creative||!['active','paused'].includes(creative.status))return;
  ...
  confirmDeleteDeliveryObject(type,id){
    const labels={plan:'广告计划',group:'广告组',creative:'广告创意'},label=labels[type];if(!label)return;
    const item=type==='plan'?DB.campaigns.find(x=>x.id===id):type==='group'?(DB.adGroups||[]).find(x=>x.id===id):DB.creatives.find(x=>x.id===id);if(!item)return;
    const cp=type==='plan'?item:DB.campaigns.find(c=>c.id===(item.camp||''));if(cp?.mode!=='rtb'){this.toast('CPD 投放对象由运营管理，广告主不可删除','info');return;}
  ...
  saveGroupEdit(){const g=(DB.adGroups||[]).find(x=>x.id===this.editGroupId);if(!g)return;const old=g.name,name=document.getElementById('editGroupName').value.trim(),bid=Number(document.getElementById('editGroupBid').value||0);if(!name||!bid){this.toast('请填写广告组名称和出价','warn');return;}g.name=name;g.bidType=document.getElementById('editGroupBidType').value;g.bid=bid;g.inventory=document.getElementById('editGroupInventory').value;DB.creatives.filter(a=>a.groupId===g.id||(!a.groupId&&a.camp===g.camp&&a.group===old)).forEach(a=>a.group=name);this.save();this.closeModal();this.go(this.editGroupReturnPage||'groupdetail');this.editGroupReturnPage=null;this.toast('广告组已更新');},
  ...
  renderAds(f){
    ...
    document.getElementById('adBody').innerHTML = list.map(a=>{
      ...
      return `<tr>
        <td><div class="cell-flex">${creativeThumb(a)}<div><div class="cell-main"><a href="javascript:void(0)" onclick="App.openCreativePreview('${a.id}')" style="color:var(--primary);text-decoration:none;cursor:pointer">${a.name}</a></div><div class="cell-sub">${a.id} · ${a.headline||'未填写文案'}</div></div></div></td>
  ...
  toggleAd(id){
    ...
    this.confirmCreativeStatus(id,'legacyAds');
  },
  openCreative(id,returnPage){
    const a=DB.creatives.find(x=>x.id===id); if(!a) return;
    const cp=DB.campaigns.find(c=>c.id===a.camp)||{};
    if(cp.mode==='cpd'){ this.openCpdCreativeEdit(id,returnPage); return; }
    this.openRtbCreativeEdit(id,returnPage);
  },
  openRtbCreativeEdit(id,returnPage){const a=DB.creatives.find(x=>x.id===id);if(!a)return;const g=(DB.adGroups||[]).find(x=>x.id===a.groupId)||{};this.editFlow={type:'creative',id,returnPage:returnPage||(this.curGroup?'groupdetail':'campdetail')};this.curGroup=g.id||this.curGroup;this.curCamp=a.camp;this.go('newplan');},
  ...
  openCpdCreativeEdit(id,returnPage){
    const a=DB.creatives.find(x=>x.id===id);if(!a)return;
    const g=(DB.adGroups||[]).find(x=>x.id===a.groupId)||{};
    this.editFlow={type:'creative',id,cpd:true,returnPage:returnPage||(this.curGroup?'groupdetail':'campdetail')};
    this.curGroup=g.id||this.curGroup;this.curCamp=a.camp;this.go('newplan');
  },
    ...
    return `<div class="unified-create">
      ...
      <div class="unified-paper">
        ...
        <section class="paper-level stage-hidden" id="uf-creative" data-level="03"><div class="level-heading"><span>03</span><div><h2>广告创意</h2><p>选择素材，并设置文案和目标链接</p></div><em>当前步骤</em></div>
          <div class="paper-block"><h3>创意内容</h3><div class="field"><label>广告创意名称<span class="req">*</span></label><input class="input" id="ufCreativeName" placeholder="如：欧洲夏促信息流 A"></div><div class="field"><label>主素材<span class="req">*</span></label><div class="flex between" style="margin-bottom:8px"><span class="cell-sub">仅显示符合当前广告组规格的素材</span><button class="btn btn-ghost btn-sm" onclick="App.openUfUpload()">${svg(I.upload)}上传新素材</button></div><div class="asset-grid" id="ufAssets">${assets.map((f,i)=>`<div class="asset-cell ${i===0?'picked':''}" data-id="${f.id}" onclick="App.pickCompatibleAsset(this)"><div class="asset-thumb" style="height:64px">${svg(I.image)}</div><div class="asset-meta"><div class="asset-name">${f.name}</div><div class="asset-sub">${f.dim} · ${f.size}</div></div></div>`).join('')}</div></div><div class="field"><label>标题 / 文案</label><input class="input" id="ufHeadline" maxlength="100" placeholder="选填，最多100字"></div><div class="field"><label>描述</label><textarea class="input" id="ufDescription" maxlength="200" placeholder="选填，最多200字"></textarea></div><div class="field"><label>CTA文案</label><input class="input" id="ufCta" maxlength="20" placeholder="选填，最多20字"></div><div class="input-row"><div class="field"><label>跳转类型</label><select class="select" id="ufJump"><option>外部跳转</option></select></div><div class="field"><label>目标链接<span class="req">*</span></label><input class="input mono" id="ufLanding" maxlength="500" placeholder="https://example.com/landing"></div></div></div><div class="level-actions"><span>提交时统一校验三个层级，并创建当前投放</span><button class="btn btn-ghost" onclick="App.cancelUnified()">取消创建</button><button class="btn btn-primary" onclick="App.saveUfCreative()">创建投放并提交审核</button></div>
        </section>
      </div></main>
    </div>`;
  },
  ...
  fillUfCreative(creative){
    ...
    this.setUfValue('ufCreativeName',data.name||creative.name);this.setUfValue('ufHeadline',data.headline||creative.headline||'');this.setUfValue('ufDescription',data.description||creative.description||'');this.setUfValue('ufLanding',data.landing||creative.landing||'');this.setUfValue('ufCta',data.cta??creative.cta??'');
  ...
  mountUnifiedEditActions(hint,saveLabel,saveAction){
    const cancel=document.querySelector('.unified-head button');if(cancel)cancel.style.display='none';
    document.querySelector('.unified-edit-actions')?.remove();
    const create=document.querySelector('.unified-create');
    if(create)create.insertAdjacentHTML('afterend',`<div class="unified-edit-actions"><span>${hint}</span><button class="btn btn-ghost" onclick="App.cancelUnified()">取消编辑</button><button class="btn btn-primary" onclick="${saveAction}">${saveLabel}</button></div>`);
  },
  initUnifiedEditFlow(){
    ...
    if(flow.type==='group'){
      ...
      const hasCreatives=DB.creatives.some(a=>a.groupId===group.id||(!a.groupId&&a.group===group.name));
      if(hasCreatives){
  ...
  saveUnifiedParentGroup(){
    const g=(DB.adGroups||[]).find(x=>x.id===this.curGroup),data=this.readUfGroup(g);if(!g||!data)return;const old=g.name;Object.assign(g,data);DB.creatives.filter(a=>a.groupId===g.id||(!a.groupId&&a.camp===g.camp&&a.group===old)).forEach(a=>a.group=g.name);this.ufWorking.group={...g};this.save();
  ...
  saveUnifiedCreativeEdit(){
    ...
    const data={name,headline:this.ufVal('ufHeadline'),description:this.ufVal('ufDescription'),cta:this.ufVal('ufCta'),landing,assetId:assetEl.dataset.id,status:'review',submittedAt:new Date().toISOString()};
  ...
  toggleUfGroupAdvanced(){
    ...
    this.toast(open?'已展开广告组独立设置':'已收起设置，独立配置保持不变');
  },
  openUfUpload(){
  ...
  saveUfCreative(){
    ...
    this.ufWorking.creative={name,assetId:file.id,headline:this.ufVal('ufHeadline'),description:this.ufVal('ufDescription'),cta:this.ufVal('ufCta'),landing};
    this.submitUnified();
  },
  submitUnified(){
  ...
  after_newad(){
    ...
    if(this.curGroup&&!edit){this.after_newplan();this.initStandaloneCreativeCreate();return;}
  ...
  view_newad(){
    const edit=DB.creatives.find(c=>c.id===this.editRejectedId && c.status==='rejected');
    if(this.curGroup && !edit) return this.view_unifiednew();
  ...
  initStandaloneCreativeCreate(){
    const cp=DB.campaigns.find(c=>c.id===this.curCamp),group=(DB.adGroups||[]).find(g=>g.id===this.curGroup);
    if(!cp||!group){this.toast('未找到所属广告计划或广告组','warn');this.go('plans');return;}
    const title=document.querySelector('.unified-head h1'),desc=document.querySelector('.unified-head p');
    if(title)title.textContent='新建广告创意';
    if(desc)desc.textContent='广告计划和广告组已完成，继续设置广告创意；提交前请检查素材与跳转信息';
    document.getElementById('topTitle').textContent='新建广告创意';
    document.getElementById('topSub').textContent='第三层：素材、文案与跳转';
    document.querySelector('.unified-create')?.classList.add('standalone-creative-create');
    const reviewNav=document.querySelector('#unifiedToc [data-target="uf-review"]');
    if(reviewNav){const rule=reviewNav.previousElementSibling;if(rule?.classList.contains('toc-rule'))rule.remove();reviewNav.remove();}
    document.getElementById('uf-review')?.remove();
    this.ufWorking={campaign:{...cp},group:{...group},existingCampaignId:cp.id,existingGroupId:group.id};
    ...
    const creative=document.getElementById('uf-creative');
    creative?.querySelector('.level-actions span')?.replaceChildren(document.createTextNode('提交后当前广告创意进入审核'));
    const creativeButtons=creative?.querySelectorAll('.level-actions button');
  ...
  submitStandaloneUnifiedCreative(){
    const cp=DB.campaigns.find(c=>c.id===this.curCamp)||{},group=(DB.adGroups||[]).find(g=>g.id===this.curGroup)||{};
    const name=this.ufVal('ufCreativeName'),landing=this.ufVal('ufLanding'),asset=document.querySelector('#ufAssets .picked');
    ...
    DB.creatives.unshift({id,name,group:group.name,groupId:group.id,assetId:file.id,fmt:group.format||'feed',kind:file.type||'image',size:file.dim||'—',camp:cp.id,headline:this.ufVal('ufHeadline'),description:this.ufVal('ufDescription'),cta:this.ufVal('ufCta'),landing,imps:0,clicks:0,ctr:0,status:'review',version:1,created:new Date().toISOString().slice(0,10)});
  ...
  submitCreativeV1(){
    ...
    DB.creatives.unshift({id,name,group:group.name,groupId:group.id,assetId:file.id,fmt:group.format||'feed',kind:file.type||'image',size:file.dim||'—',camp:cp.id,headline:document.getElementById('creativeHeadline').value.trim(),landing,imps:0,clicks:0,ctr:0,status:'review',version:1,created:new Date().toISOString().slice(0,10)});
  ...
  view_report(){
    ...
    <div class="card"><div class="card-head"><h3 id="repTitle">按格式明细</h3><span class="badge gray rep-range-label">近 7 天</span><div class="spacer"></div>
      ...
      <button class="btn btn-ghost btn-sm" style="margin-left:8px" onclick="App.exportRtbReport()">${svg(I.download)}导出</button></div>
  ...
  repFactor(){ return (this.repDays||7)/7; },
  repRangeMeta(){
  ...
  repDim(dim){
    ...
    if(dim==='inv'){
      ...
      html=rows.map(r=>{ const cpc=r[2]?fmtMoney(r[3]/r[2]):'$0.00'; const cpm=r[1]?fmtMoney(r[3]/r[1]*1000):'$0.00'; return `<tr><td class="cell-main">${r[0]}</td><td class="num">${fmtNum(r[1])}</td><td class="num">${fmtNum(r[2])}</td><td class="num">${r[1]?(r[2]/r[1]*100).toFixed(2)+'%':'—'}</td><td class="num">${cpc}</td><td class="num">${cpm}</td><td class="num cell-main">${fmtMoney(r[3])}</td></tr>`; }).join('');
    }
