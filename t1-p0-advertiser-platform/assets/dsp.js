...
const App = {
  ...
  view_groupdetail(){
    ...
    <div class="card group-setting-summary">
      <div class="group-summary-title"><b>投放设置</b><span class="badge ${groupIssue.cls}">${groupIssue.label}</span>${groupIssue.key!=='ready'?`<span class="group-issue-reason">${deliveryText}</span>`:''}</div>
      <div class="group-summary-items">
        <span><em>投放周期</em><strong>${group.start||cp.start||'—'} 至 ${group.end||cp.end||'长期投放'}</strong></span>
        <span><em>${isRtb?'广告组预算':'预算'}</em><strong>${isRtb?fmtMoney(group.budget||0):'由 CPD 订单管理'}</strong></span>
        ${isRtb?`<span><em>每日上限</em><strong>${group.dailyCap?fmtMoney(group.dailyCap):'未单独设置'}</strong></span>`:''}
        <span><em>计费与出价</em><strong>${isRtb?`${group.bidType||'CPM'} · ${fmtMoney(group.bid||0)}`:'CPD 代投'}</strong></span>
      </div>
      <button class="text-link group-summary-edit" onclick="App.openGroupEdit('${group.id}')">查看或修改投放设置</button>
    </div>
