...
const App = {
  ...
  renderUnifiedPlans(){
    ...
    document.getElementById('unifiedPlanBody').innerHTML=list.map(cp=>{
      ...
      const action=cp.status==='draft'?`<button class="text-link plan-row-action" onclick="App.openPlan('${cp.id}')">继续创建</button>`:isCpd?'<span class="na-value">—</span>':`<button class="text-link plan-row-action" onclick="App.openPlanActions('${cp.id}')">更多</button>`;
