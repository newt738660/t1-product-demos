...
const App = {
  ...
  view_groupdetail(){
    ...
    <div class="notice ${groupIssue.cls==='red'?'danger':groupIssue.cls==='amber'?'warn':'info'}" style="margin-bottom:16px"><b>${groupIssue.label}：</b>${deliveryText}</div>
    <div class="grid" style="grid-template-columns:repeat(3,minmax(0,1fr));margin-bottom:16px">
      <div class="card kpi"><div class="kpi-label">投放周期</div><div class="kpi-val" style="font-size:15px;margin-top:10px">${group.start||cp.start||'—'} 至 ${group.end||cp.end||'长期投放'}</div></div>
      <div class="card kpi"><div class="kpi-label">预算</div><div class="kpi-val" style="font-size:15px;margin-top:10px">${isRtb?`${fmtMoney(group.budget||0)}<div class="cell-sub">日预算 ${group.dailyCap?fmtMoney(group.dailyCap):'未单独设置'}</div>`:'由 CPD 订单管理'}</div></div>
      <div class="card kpi"><div class="kpi-label">计费方式 / 出价</div><div class="kpi-val" style="font-size:15px;margin-top:10px">${isRtb?`${group.bidType||'CPM'} · ${fmtMoney(group.bid||0)}`:'CPD 代投'}</div></div>
      <div class="card kpi"><div class="kpi-label">广告形式 / 库存</div><div class="kpi-val" style="font-size:15px;margin-top:10px">${FMT[group.format]?.name||'—'}<div class="cell-sub">${group.inventory||cp.placement||'—'}</div></div></div>
      <div class="card kpi"><div class="kpi-label">地域定向</div><div class="kpi-val" style="font-size:15px;margin-top:10px">${group.geo||'不限'}</div></div>
      <div class="card kpi"><div class="kpi-label">设备定向</div><div class="kpi-val" style="font-size:15px;margin-top:10px">${group.device||'不限'}<div class="cell-sub">共 ${ads.length} 条创意</div></div></div>
    </div>
    <div class="card"><div class="card-head"><div><b>广告创意</b><div class="cell-sub">审核状态说明内容是否可用；投放状态同时受计划、广告组和创意自身状态影响</div></div>${isRtb?`<div class="spacer"></div><button class="btn btn-primary btn-sm" onclick="App.openNewCreativeForGroup('${group.id}')">${svg(I.plus)}新建创意</button>`:''}</div><div class="table-wrap"><table><thead><tr><th>广告创意</th><th>素材规格</th><th>审核状态</th><th>投放状态</th><th class="num">曝光</th><th class="num">点击</th><th class="num">CTR</th><th>更新时间</th><th>操作</th></tr></thead><tbody>${ads.map(a=>{const ctr=a.imps?((a.clicks||0)/a.imps*100).toFixed(2)+'%':'—',creativeIssue=this.creativeDeliveryIssue(a,group,cp),reviewState=a.pendingVersion?'<span class="badge green">当前版本已通过</span><div class="cell-sub" style="margin-top:4px">新版本审核中</div>':this.statusBadge(a.status),canToggle=isRtb&&['active','paused'].includes(a.status),toggleLabel=a.status==='paused'?'恢复':'暂停',updated=a.pendingVersion?.submittedAt?new Date(a.pendingVersion.submittedAt).toLocaleDateString('zh-CN'):a.updated||a.created||'—';return `<tr><td><div class="cell-flex">${creativeThumb(a)}<div><button class="link-btn" onclick="App.openCreative('${a.id}')"><b>${a.name}</b></button><div class="cell-sub">${a.id} · ${a.headline||'未填写标题'}</div></div></div></td><td>${a.size||'—'}</td><td>${reviewState}</td><td><span class="badge ${creativeIssue.cls}">${creativeIssue.label}</span></td><td class="num">${fmtK(a.imps||0)}</td><td class="num">${fmtK(a.clicks||0)}</td><td class="num">${ctr}</td><td>${updated}</td><td><div class="cell-flex"><button class="btn btn-ghost btn-sm" onclick="App.openCreative('${a.id}')">查看 / 编辑</button>${canToggle?`<button class="btn btn-ghost btn-sm" onclick="App.toggleCreativeStatus('${a.id}')">${toggleLabel}</button>`:''}</div></td></tr>`}).join('')||`<tr><td colspan="9"><div class="empty">${svg(I.creative)}<div>当前广告组还没有广告创意</div>${isRtb?`<button class="btn btn-primary btn-sm" onclick="App.openNewCreativeForGroup('${group.id}')">创建第一条创意</button>`:''}</div></td></tr>`}</tbody></table></div></div>`;
  },
  toggleGroupStatus(id){
    ...
    this.save();this.go('groupdetail');this.toast(next==='paused'?'广告组已暂停；下属创意自身状态保持不变':'广告组已恢复；符合条件的创意可继续投放');
  },
