...
const App = {
  ...
  view_groupdetail(){
    ...
    <div class="card"><div class="card-head"><div><b>当前广告组下的广告创意</b><div class="cell-sub">列表展示当前正在投放的内容和数据；已提交的修改在“内容变更”中单独提示</div></div>${cp.mode==='rtb'?`<div class="spacer"></div><button class="btn btn-primary btn-sm" onclick="App.openNewCreativeForGroup('${group.id}')">${svg(I.plus)}新建创意</button>`:''}</div><div class="table-wrap"><table><thead><tr><th>广告创意</th><th>素材规格</th><th>状态</th><th>内容变更</th><th>投放情况</th><th class="num">曝光</th><th class="num">点击</th><th class="num">CTR</th><th>落地页</th><th>操作</th></tr></thead><tbody>${ads.map(a=>{const ctr=a.imps?((a.clicks||0)/a.imps*100).toFixed(2)+'%':'—',creativeIssue=this.creativeDeliveryIssue(a,group,cp),actionLabel=a.status==='rejected'?'查看驳回原因 / 修改':a.pendingVersion?'查看投放内容与本次修改':cp.mode==='cpd'?'查看 / 修改':'查看 / 编辑';return `<tr><td><div class="cell-flex">${creativeThumb(a)}<div><b>${a.name}</b><div class="cell-sub">${a.id} · ${a.headline||'未填写标题'}</div></div></div></td><td>${a.size||'—'}</td><td>${this.statusBadge(a.status)}</td><td>${a.pendingVersion?'<span class="badge amber">修改审核中</span>':'<span class="cell-sub">无待审修改</span>'}</td><td><span class="badge ${creativeIssue.cls}">${creativeIssue.label}</span></td><td class="num">${fmtK(a.imps||0)}</td><td class="num">${fmtK(a.clicks||0)}</td><td class="num">${ctr}</td><td class="cell-sub">${a.landing||'—'}</td><td><button class="btn btn-ghost btn-sm" onclick="App.openCreative('${a.id}')">${actionLabel}</button></td></tr>`}).join('')||`<tr><td colspan="10"><div class="empty">${svg(I.creative)}<div>当前广告组还没有广告创意</div>${cp.mode==='rtb'?`<button class="btn btn-primary btn-sm" onclick="App.openNewCreativeForGroup('${group.id}')">创建第一条创意</button>`:''}</div></td></tr>`}</tbody></table></div></div>`;
  },
  ...
  renderAds(f){
    ...
    document.getElementById('adBody').innerHTML = list.map(a=>{
      ...
      return `<tr>
        <td><div class="cell-flex">${creativeThumb(a)}<div><div class="cell-main"><a href="javascript:void(0)" onclick="App.openCreative('${a.id}')" style="color:var(--primary);text-decoration:none;cursor:pointer">${a.name}</a></div><div class="cell-sub">${a.id} · ${a.headline||'未填写文案'}</div></div></div></td>
  ...
  openCpdCreativeEdit(id){
    ...
    this.modal(`
      ...
      <div class="modal-body">
        ...
        ${a.pendingVersion?`<div class="notice info" style="margin-bottom:16px"><b>当前内容仍在投放</b><div style="margin-top:6px">你提交的修改正在审核；审核通过前，当前投放内容和数据保持不变。</div></div><div class="grid cols-2" style="margin-bottom:16px"><div class="card card-pad"><div class="cell-sub">当前投放内容</div><b style="display:block;margin-top:8px">${a.name}</b><div style="margin-top:6px">${a.headline||'未填写标题'}</div><div class="cell-sub" style="margin-top:6px">${a.landing||'—'}</div></div><div class="card card-pad"><div class="cell-sub">本次提交的修改</div><b style="display:block;margin-top:8px">${a.pendingVersion.name}</b><div style="margin-top:6px">${a.pendingVersion.headline||'未填写标题'}</div><div class="cell-sub" style="margin-top:6px">${a.pendingVersion.landing||'—'}</div><span class="badge amber" style="margin-top:8px">修改审核中</span></div></div>`:''}
