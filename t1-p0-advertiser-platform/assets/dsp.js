...
const App = {
  ...
  openCreativePreview(id){
    ...
    this.modal(`<div class="modal-head"><div><h3>${a.name}</h3><p>${a.id} · 当前生效版本 V${a.version||1}</p></div><div class="spacer"></div><span class="badge ${issue.cls}">${issue.label}</span><button class="icon-btn" onclick="App.closeModal()">${svg(I.x)}</button></div><div class="modal-body">${a.pendingVersion?`<div class="notice info" style="margin-bottom:16px"><b>新版本审核中</b><div style="margin-top:5px">当前展示的是仍在投放的 V${a.version||1}，审核通过后才会替换。</div></div>`:''}<div class="creative-preview-modal"><div class="creative-preview-large">${creativeThumb(a)}<span>${asset.name||a.size||'当前素材'}</span></div><div><dl class="cpd-readonly-grid"><div class="wide"><dt>所属广告计划</dt><dd>${cp.name||'—'}</dd></div><div class="wide"><dt>所属广告组</dt><dd>${group.name||a.group||'—'}</dd></div><div><dt>素材规格</dt><dd>${a.size||asset.dim||'—'}</dd></div><div><dt>曝光</dt><dd>${fmtK(a.imps||0)}</dd></div><div><dt>点击 / CTR</dt><dd>${fmtK(a.clicks||0)} / ${ctr}</dd></div><div><dt>广告形式</dt><dd>${FMT[a.fmt]?.name||'—'}</dd></div><div class="wide"><dt>标题 / 文案</dt><dd>${a.headline||'—'}</dd></div><div class="wide"><dt>补充文案</dt><dd>${a.description||'—'}</dd></div><div><dt>跳转类型</dt><dd>外部跳转</dd></div><div class="wide"><dt>目标链接</dt><dd class="mono">${a.landing||'—'}</dd></div></dl></div></div></div><div class="modal-foot"><div class="spacer"></div><button class="btn btn-primary" onclick="App.closeModal()">关闭</button></div>`,true);
  },
