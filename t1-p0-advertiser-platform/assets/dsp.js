...
const DB = {
  ...
  creatives: [
    ...
    {id:'CR-3007',scenario:'state-case',name:'游戏预约开屏',group:'游戏开屏｜预约期',groupId:'G-2005',fmt:'splashimg',kind:'image',size:'1080×1920',camp:'C-1005',headline:'预约游戏新版本',description:'当前线上生效内容',landing:'https://www.t1t1.com/game',imps:228000,clicks:1368,status:'active',version:1,created:'2026-08-15',changeStatus:'review',pendingVersion:{version:2,name:'游戏预约开屏 V2',headline:'立即预约新版本',description:'全新角色预约即得好礼',jumpType:'external',landing:'https://www.t1t1.com/game-v2',status:'review',submittedAt:'2026-08-17T10:20:00Z'}},
...
const App = {
  ...
  unifiedPlanIssue(cp,creatives){
    ...
    if(creatives.some(a=>a.status==='review'||a.pendingVersion?.status==='review'))return {key:'review',label:'创意审核中',cls:'amber'};
  ...
  view_campdetail(){
    ...
    <div class="notice ${planIssue.cls==='red'?'warning':'info'}" style="margin-bottom:14px"><div class="flex" style="gap:10px;align-items:flex-start"><span class="badge ${planIssue.cls}">${planIssue.label}</span><div><b>当前投放情况</b><div style="margin-top:4px">${planIssueText}</div></div></div></div>
    ...
    <div class="card group-list-card"><div class="card-head"><div><b>广告组</b><div class="cell-sub">共 <span id="visibleGroupCount">${groups.length}</span> 个广告组；点击名称进入广告组详情</div></div><div class="spacer"></div>${groups.length?`<input class="input group-search" id="campaignGroupSearch" placeholder="搜索广告组名称 / ID" oninput="App.filterCampaignGroups()"><select class="select group-status-filter" id="campaignGroupStatus" onchange="App.filterCampaignGroups()"><option value="">全部状态</option><option value="active">投放中</option><option value="draft">草稿</option><option value="review">审核中</option><option value="needs-creative">待创建创意</option></select>`:''}</div>
      <div class="table-wrap"><table><thead><tr><th>广告组</th><th>状态</th><th>投放情况</th><th>排期</th><th>${cp.mode==='cpd'?'广告形式 / 库存':'预算 / 竞价'}</th><th class="num">创意</th><th class="num">曝光</th><th class="num">点击</th><th class="num">CTR</th></tr></thead><tbody id="campaignGroupRows">
      ${groups.map(g=>{const ga=ads.filter(a=>a.groupId===g.id||a.group===g.name),gi=ga.reduce((s,a)=>s+(a.imps||0),0),gc=ga.reduce((s,a)=>s+(a.clicks||0),0),gctr=gi?(gc/gi*100).toFixed(2)+'%':'—',needsCreative=ga.length===0,groupIssue=this.groupDeliveryIssue(g,cp,ga);return `<tr data-group-row data-search="${(g.name+' '+g.id).toLowerCase()}" data-status="${needsCreative?'needs-creative':g.status}"><td><div class="cell-main" style="color:var(--accent);cursor:pointer" onclick="App.openGroupDetail('${g.id}','${g.name.replaceAll("'","&#39;")}')">${g.name}</div><div class="cell-sub">${g.id}</div></td><td>${this.statusBadge(g.status)}</td><td><span class="badge ${groupIssue.cls}">${groupIssue.label}</span></td><td>${g.start||cp.start||'—'}<div class="cell-sub">${g.end?'至 '+g.end:'长期投放'}</div></td><td>${cp.mode==='cpd'?`${FMT[g.format]?.name||'—'}<div class="cell-sub">${g.inventory||cp.placement||'—'}</div>`:`${fmtMoney(g.budget||0)}<div class="cell-sub">${g.bidType||'CPM'} · ${fmtMoney(g.bid||0)}</div>`}</td><td class="num">${needsCreative?(cp.mode==='rtb'?`<button class="text-link" onclick="App.openNewCreativeForGroup('${g.id}')">创建创意</button>`:'<span class="badge amber">0 · 待补充</span>'):ga.length}</td><td class="num">${fmtK(gi)}</td><td class="num">${fmtK(gc)}</td><td class="num">${gctr}</td></tr>`}).join('')||`<tr><td colspan="9"><div class="empty">${svg(I.group||I.camp)}<div>当前广告计划还没有广告组</div>${cp.mode==='rtb'?`<button class="btn btn-primary btn-sm" onclick="App.startNewGroup('${cp.id}')">创建第一个广告组</button>`:''}</div></td></tr>`}
      </tbody></table></div></div>`;
  },
  ...
  view_groupdetail(){
    ...
    <div class="grid" style="grid-template-columns:repeat(5,minmax(0,1fr));margin-bottom:16px"><div class="card kpi"><div class="kpi-label">自身状态</div><div style="margin-top:10px">${this.statusBadge(group.status)}</div></div><div class="card kpi"><div class="kpi-label">投放情况</div><div style="margin-top:10px"><span class="badge ${groupIssue.cls}">${groupIssue.label}</span></div></div><div class="card kpi"><div class="kpi-label">投放周期</div><div class="kpi-val" style="font-size:15px;margin-top:10px">${group.start||cp.start||'—'} ${group.end?'至 '+group.end:'起长期'}</div></div><div class="card kpi"><div class="kpi-label">广告形式 / 库存</div><div class="kpi-val" style="font-size:15px;margin-top:10px">${FMT[group.format]?.name||cp.placement||'—'}</div></div><div class="card kpi"><div class="kpi-label">创意数</div><div class="kpi-val">${ads.length}</div></div></div>
    <div class="card"><div class="card-head"><div><b>当前广告组下的广告创意</b><div class="cell-sub">列表主信息与投放数据均对应当前线上生效版本；待审版本在“状态 / 版本”中单独展示</div></div>${cp.mode==='rtb'?`<div class="spacer"></div><button class="btn btn-primary btn-sm" onclick="App.openNewCreativeForGroup('${group.id}')">${svg(I.plus)}新建创意</button>`:''}</div><div class="table-wrap"><table><thead><tr><th>广告创意（当前线上版本）</th><th>素材规格</th><th>状态 / 版本</th><th>投放情况</th><th class="num">曝光</th><th class="num">点击</th><th class="num">CTR</th><th>落地页</th><th>操作</th></tr></thead><tbody>${ads.map(a=>{const ctr=a.imps?((a.clicks||0)/a.imps*100).toFixed(2)+'%':'—',creativeIssue=this.creativeDeliveryIssue(a,group,cp),actionLabel=a.status==='rejected'?'查看驳回原因 / 修改':a.pendingVersion?'查看当前与待审版本':cp.mode==='cpd'?'查看 / 修改':'查看 / 编辑';return `<tr><td><div class="cell-flex"><span class="row-ico" style="color:${(FMT[a.fmt]||{}).color}">${svg((FMT[a.fmt]||{}).ico||I.image)}</span><div><b>${a.name}</b><div class="cell-sub">${a.id} · ${a.headline||'未填写标题'}</div></div></div></td><td>${a.size||'—'}</td><td>${this.statusBadge(a.status)}<div class="cell-sub">当前 V${a.version||1}${a.pendingVersion?` · 已提交 V${a.pendingVersion.version} 审核中`:''}</div></td><td><span class="badge ${creativeIssue.cls}">${creativeIssue.label}</span></td><td class="num">${fmtK(a.imps||0)}</td><td class="num">${fmtK(a.clicks||0)}</td><td class="num">${ctr}</td><td class="cell-sub">${a.landing||'—'}</td><td><button class="btn btn-ghost btn-sm" onclick="App.openCreative('${a.id}')">${actionLabel}</button></td></tr>`}).join('')||`<tr><td colspan="9"><div class="empty">${svg(I.creative)}<div>当前广告组还没有广告创意</div>${cp.mode==='rtb'?`<button class="btn btn-primary btn-sm" onclick="App.openNewCreativeForGroup('${group.id}')">创建第一条创意</button>`:''}</div></td></tr>`}</tbody></table></div></div>`;
  },
  ...
  openRtbCreativeEdit(id){const a=DB.creatives.find(x=>x.id===id);if(!a)return;this.editCreativeId=id;this.modal(`<div class="modal-head"><div><h3>编辑广告创意</h3><p>${a.id} · 修改后重新提交审核</p></div><div class="spacer"></div><button class="icon-btn" onclick="App.closeModal()">${svg('<path d="M18 6 6 18M6 6l12 12"/>')}</button></div><div class="modal-body">${a.pendingVersion?`<div class="grid cols-2" style="margin-bottom:14px"><div class="card card-pad"><div class="cell-sub">当前线上版本 · V${a.version||1}</div><b style="display:block;margin-top:8px">${a.name}</b><div style="margin-top:6px">${a.headline||'未填写标题'}</div></div><div class="card card-pad"><div class="cell-sub">待审版本 · V${a.pendingVersion.version}</div><b style="display:block;margin-top:8px">${a.pendingVersion.name}</b><div style="margin-top:6px">${a.pendingVersion.headline||'未填写标题'}</div><span class="badge amber" style="margin-top:8px">审核中</span></div></div>`:''}<div class="notice info" style="margin-bottom:14px">提交后列表仍展示当前线上版本及其投放数据；待审版本单独保留，审核通过后再替换线上版本。</div><div class="field"><label>广告创意名称</label><input class="input" id="editCreativeName" value="${a.pendingVersion?.name||a.name}"></div><div class="field"><label>标题 / 文案</label><input class="input" id="editCreativeHeadline" value="${a.pendingVersion?.headline||a.headline||''}"></div><div class="field"><label>目标链接</label><input class="input mono" id="editCreativeLanding" value="${a.pendingVersion?.landing||a.landing||''}"></div></div><div class="modal-foot"><button class="btn btn-ghost" onclick="App.closeModal()">取消</button><div class="spacer"></div><button class="btn btn-primary" onclick="App.saveRtbCreativeEdit()">保存并提交审核</button></div>`,true);},
  ...
  openCpdCreativeEdit(id){
    ...
    this.modal(`
      ...
      <div class="modal-body">
        ...
        ${a.pendingVersion?`<div class="notice info" style="margin-bottom:16px"><b>当前线上版本仍在投放</b><div style="margin-top:6px">列表和投放数据展示 V${a.version||1}；你提交的 V${a.pendingVersion.version} 正在审核，通过后才会替换线上版本。</div></div><div class="grid cols-2" style="margin-bottom:16px"><div class="card card-pad"><div class="cell-sub">当前线上版本 · V${a.version||1}</div><b style="display:block;margin-top:8px">${a.name}</b><div style="margin-top:6px">${a.headline||'未填写标题'}</div><div class="cell-sub" style="margin-top:6px">${a.landing||'—'}</div></div><div class="card card-pad"><div class="cell-sub">待审版本 · V${a.pendingVersion.version}</div><b style="display:block;margin-top:8px">${a.pendingVersion.name}</b><div style="margin-top:6px">${a.pendingVersion.headline||'未填写标题'}</div><div class="cell-sub" style="margin-top:6px">${a.pendingVersion.landing||'—'}</div><span class="badge amber" style="margin-top:8px">审核中</span></div></div>`:''}
