  ...
  view_campdetail(){
    ...
    <div class="card group-setting-summary" aria-label="广告计划概览">
      ...
      <dl class="group-setting-grid plan-setting-grid">
        ...
        ${cp.mode==='rtb'?`<div><dt>花费</dt><dd>${fmtMoney(spend)}</dd></div><div><dt>预算</dt><dd>${fmtMoney(budget||0)}</dd></div><div><dt>每日上限</dt><dd>${cp.dailyCap?fmtMoney(cp.dailyCap):'未单独设置'}</dd></div>`:''}
      </dl>
    </div>
  ...
  view_groupdetail(){
    ...
    ${isRtb?`<div class="card group-setting-summary">
      ...
      <dl class="group-setting-grid">
        ...
        <div><dt>花费</dt><dd>${fmtMoney(groupSpend)}</dd></div>
