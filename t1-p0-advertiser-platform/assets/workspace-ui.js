(()=>{
 ...
 A.renderHomeSchedule=function(schedule){const {issues,saved}=this.homeIssueState();return schedule.length?schedule.slice(0,3).map(s=>{
  const warnings=issues.filter(a=>a.c?.id===s.c.id&&a.type==='missing'&&a.ignorable&&!saved[a.id]&&s.label==='开始投放');
  return `<div class="home-schedule-row"><div class="home-date"><b>${s.date.slice(8)}</b><span>${s.date.slice(5,7)}月</span></div><div class="home-schedule-name"><b>${E(s.c.alias||s.c.name)}</b><p>${s.c.mode.toUpperCase()} · ${s.label}${s.c.status==='paused'?' · 当前已暂停':''}</p>${warnings.map(a=>`<div class="home-schedule-warning"><span class="badge amber">开始前需补充创意</span><span class="cell-sub">广告组：${E(a.group.name)}</span><div>${button(a.label,a.call)} ${button('忽略',`App.ignoreHomeIssue(${JSON.stringify(a.id)})`,'link-btn')}</div></div>`).join('')}</div>${warnings.length?'':button('查看',`App.openPlan(${JSON.stringify(s.c.id)})`)}</div>`;
 }).join(''):'<p class="home-issue-note">未来7天暂无排期变更</p>';};
 ...
 A.view_matureDash=function(){
  ...
  return `<div class="advertiser-home home-four-home"><div class="home-heading"><h1>账户概览</h1></div>${this.renderHomeIssues()}<div class="home-bottom-grid"><section class="card home-overview-side"><div class="home-section-title"><h2>投放概况</h2>${button('管理计划',"App.go('plans')")}</div><div class="home-side-states">${states.map(([k,label])=>`<button class="home-compact-state" onclick="App.openHomePlans('${k}')"><span>${label}</span><strong>${count(k)}</strong></button>`).join('')}</div><div class="home-secondary-states">${button('已暂停 '+count('paused'),"App.openHomePlans('paused')",'link-btn')}${button('已结束 '+count('ended'),"App.openHomePlans('ended')",'link-btn')}</div>${count('unknown')?'<p class="home-side-note">部分状态更新延迟，不代表投放已停止。</p>':''}<div class="home-side-note">${date} · UTC · 可投放不代表已产生曝光</div></section><section class="card home-overview-side"><div class="home-section-title"><div><h2>近期排期</h2><p>未来7天 · ${schedule.length}项安排</p></div>${button('查看计划',"App.go('plans')")}</div>${this.renderHomeSchedule(schedule.map(x=>({...x,label:x.label==='开始'?'开始投放':'排期结束'})))}</section></div><nav class="home-quicklinks" aria-label="常用操作"><span class="cell-sub">常用入口</span>${button('数据报表',"App.go('report')",'link-btn')}${this.canOpen('creatives')?button('广告素材',"App.go('creatives')",'link-btn'):''}${this.canOpen('billing')?button('资金记录',"App.go('billing')",'link-btn'):''}${button('账户设置',"App.go('settings')",'link-btn')}${button('帮助中心',"App.go('help')",'link-btn')}</nav></div>`;
