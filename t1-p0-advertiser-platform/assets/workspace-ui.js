(()=>{
 ...
 A.toggleHomeIssueDetail=function(button){const detail=button.closest('.home-issue-row').querySelector('.home-issue-detail');detail.hidden=!detail.hidden;button.setAttribute('aria-expanded',String(!detail.hidden));button.textContent=detail.hidden?'详情':'收起';};
 A.renderHomeIssues=function(){
  ...
  const row=a=>{
   ...
   const full=object+(a.group?' · '+a.group.id:'')+(a.c?' · '+a.c.id+' · '+a.c.mode.toUpperCase():'');
   return `<div class="home-issue-row"><span class="badge gray home-issue-category">${category[a.type]}</span><div class="home-issue-copy"><b class="home-issue-title">${E(a.title)}</b><div class="home-issue-object" title="${E(full)}">${E(object)}</div></div><div class="home-issue-actions">${button(a.label,a.call)}<button class="btn btn-ghost btn-sm" aria-expanded="false" onclick="App.toggleHomeIssueDetail(this)">详情</button>${a.ignorable?button(isIgnored?'恢复':'忽略',`App.ignoreHomeIssue(${JSON.stringify(a.id)},${isIgnored})`):''}</div><div class="home-issue-detail" hidden><div>${E(full)}</div><p>${E(a.desc)}</p></div></div>`;
  };
  return `<section id="homeIssues" class="card home-issues"><div class="home-section-title home-issue-heading"><h2>需要关注与处理</h2><div class="home-issue-tabs">${button('待处理（'+active.length+'）',"App.setHomeIssueView('active')",'btn btn-sm '+(!isIgnored?'btn-primary':'btn-ghost'))}${button('已忽略（'+ignored.length+'）',"App.setHomeIssueView('ignored')",'btn btn-sm '+(isIgnored?'btn-primary':'btn-ghost'))}</div>${rows.length>3?`<button class="btn btn-ghost btn-sm" aria-expanded="${!!this.homeIssuesExpanded}" onclick="App.showHomeAlerts()">${this.homeIssuesExpanded?'收起':'展开全部（'+rows.length+'）'}</button>`:''}</div>${isIgnored?'<p class="home-issue-note">仅忽略本次风险，不改变投放状态；可恢复提醒。</p>':''}${rows.length?(this.homeIssuesExpanded?rows:rows.slice(0,3)).map(row).join(''):'<p class="home-issue-note">'+(isIgnored?'暂无已忽略事项':'暂无需要处理的事项')+'</p>'}${!isIgnored&&system.length?'<div class="home-system-list">'+system.map(row).join('')+'</div>':''}</section>`;
