(()=>{
 ...
 A.setHomeIssueView=function(view){this.homeIssueView=view;this.homeIssuesExpanded=true;this.homeOnlyBlocked=false;this.refreshHomeIssues();};
 A.refreshHomeIssues=function(){const el=document.getElementById('content');if(el&&document.getElementById('homeIssues'))el.innerHTML=this.view_matureDash();this.syncHomeBalanceRisk();};
 A.showHomeAlerts=function(blocked=false){this.homeIssuesExpanded=blocked?true:!this.homeIssuesExpanded;this.homeOnlyBlocked=blocked;this.homeIssueView='active';this.refreshHomeIssues();};
 A.ignoreHomeIssue=function(id,restore=false){const {issues,saved}=this.homeIssueState();if(!issues.some(a=>a.id===id&&a.ignorable))return;if(restore)delete saved[id];else saved[id]=true;localStorage.setItem(this.homeIgnoreKey(),JSON.stringify(saved));this.refreshHomeIssues();this.toast(restore?'已恢复提醒':'已忽略，可从投放概况的“已忽略”找回');};
 A.homeIssueRow=function(a,ignored=false){
  const category={balance:'资金与预算','balance-risk':'资金与预算',budget:'资金与预算',rejected:'创意审核',missing:'投放准备',other:'投放受限'};
  const object=a.group?`广告组：${a.group.name} · 所属计划：${a.c.name}`:a.c?`计划：${a.c.name}`:'账户：'+(this.profile()?.advertiserName||'当前广告主');
  const call=a.group?`App.openGroupDetail(${JSON.stringify(a.group.id)})`:a.c?`App.openPlan(${JSON.stringify(a.c.id)})`:"App.go('billing')";
  const desc=a.type==='budget'?'计划预算耗尽，调整后重新检查投放条件。':a.type==='rejected'?'该广告组无可用创意，请按驳回意见修改后重提。':a.type==='balance-risk'?'按近7个完整日平均消耗估算，余额预计不足2天。':a.desc;
  return `<div class="home-four-row"><div><span class="badge gray">${category[a.type]||'投放提示'}</span></div><div class="home-four-problem"><b>${E(a.title)}</b><div class="cell-sub">${E(desc)}</div></div><div class="home-four-object"><button class="link-btn home-object-link" title="${E(object+' · '+(a.group?.id||a.c?.id||''))}" onclick="${E(call)}">${E(a.group?'广告组：'+a.group.name:a.c?'计划：'+a.c.name:object)}</button>${a.group?`<div class="cell-sub">所属计划：${E(a.c.name)}</div>`:''}</div><div class="home-four-actions">${button(a.label,a.call)}${a.ignorable?button(ignored?'恢复':'忽略',`App.ignoreHomeIssue(${JSON.stringify(a.id)},${ignored})`,'link-btn'):''}</div></div>`;

 };
 A.renderHomeIssues=function(){
  const {issues,saved}=this.homeIssueState(),plans=DB.campaigns.filter(c=>c.status!=='archived'),count=k=>plans.filter(c=>this.homePlanState(c)===k).length;
  const blocked=plans.filter(c=>this.homePlanState(c)==='blocked'),active=issues.filter(a=>a.priority===0&&!saved[a.id]),ignored=issues.filter(a=>saved[a.id]);
  // A blocked plan must remain reachable even when it has no dedicated actionable alert.
  for(const c of blocked)if(!active.some(a=>a.c?.id===c.id||a.type==='balance'&&c.mode==='rtb'))active.push({id:'other:'+c.id,type:'other',c,title:'暂无可投放的广告组或创意',desc:'查看广告组与创意的当前状态，确认限制原因。',label:'查看计划',call:`App.openPlan(${JSON.stringify(c.id)})`});
  const rows=this.homeIssueView==='ignored'?ignored:active;
  const expanded=!!this.homeIssuesExpanded||active.length===1;
  if(!active.length&&!ignored.length)return '<div id="homeIssues" hidden></div>';
  const title=blocked.length?`${blocked.length}个计划无法投放，需要处理`:active.length?`${active.length}项问题影响部分投放，需要处理`:'已忽略提醒';
  const severity=blocked.length?'danger':active.length?'warning':'neutral';
  return `<section id="homeIssues" class="card home-four-issues"><div class="home-summary home-summary-${severity}"><div class="home-summary-copy"><strong>${active.length?'⚠ ':''}${title}</strong></div>${active.length>1?`<button class="btn btn-primary btn-sm" aria-expanded="${expanded}" onclick="App.showHomeAlerts()">${expanded?'收起问题':'查看并处理（'+active.length+'）'}</button>`:''}${ignored.length?button('已忽略（'+ignored.length+'）',"App.setHomeIssueView('ignored')",'link-btn'):''}</div>${expanded?`<div class="home-expanded-tools">${button('待处理（'+active.length+'）',"App.setHomeIssueView('active')",'btn btn-sm '+(this.homeIssueView!=='ignored'?'btn-primary':'btn-ghost'))}${button('已忽略（'+ignored.length+'）',"App.setHomeIssueView('ignored')",'btn btn-sm '+(this.homeIssueView==='ignored'?'btn-primary':'btn-ghost'))}</div><div class="home-four-head"><span>问题类别</span><span>问题与影响</span><span>受影响对象</span><span>处理动作</span></div>${rows.map(a=>this.homeIssueRow(a,this.homeIssueView==='ignored')).join('')||'<p class="home-side-note">暂无事项</p>'}`:''}</section>`;
 };

 A.renderHomeSchedule=function(schedule){const {issues,saved}=this.homeIssueState();return schedule.length?`<div class="home-schedule-table"><div class="home-schedule-columns home-schedule-header"><span>日期（UTC）</span><span>广告计划</span><span>排期事项</span><span>操作</span></div>${schedule.slice(0,3).map(s=>{
  const warnings=issues.filter(a=>a.c?.id===s.c.id&&a.type==='missing'&&a.ignorable&&!saved[a.id]&&s.label==='开始投放'),one=warnings.length===1?warnings[0]:null;
  return `<div class="home-schedule-columns home-schedule-item"><time>${s.date}</time><div class="home-schedule-plan"><button class="link-btn" onclick="App.openPlan(${E(JSON.stringify(s.c.id))})">${E(s.c.alias||s.c.name)}</button><div class="cell-sub">${s.c.mode.toUpperCase()} · ${E(s.c.id)}</div></div><div class="home-schedule-event">${s.label}${s.c.status==='paused'?' · 已暂停':''}${warnings.length?`<div class="cell-sub">${one?'广告组：'+E(one.group.name):warnings.length+'个广告组'} · <span class="home-schedule-risk">开始前需补充创意</span></div>`:''}</div><div class="home-schedule-ops">${one?button(one.label,one.call):button(warnings.length?'准备创意':'查看计划',`App.openPlan(${JSON.stringify(s.c.id)})`)}${one?button('忽略',`App.ignoreHomeIssue(${JSON.stringify(one.id)})`,'link-btn'):''}</div></div>`;
 }).join('')}</div>`:'<p class="home-side-note">未来7天暂无排期安排</p>';};
 A.syncHomeBalanceRisk=function(){document.getElementById('homeBalanceRisk')?.remove();if(!this.isAdvertiserBound()||!this.canOpen('billing'))return;const {issues,saved}=this.homeIssueState(),risk=issues.find(a=>a.type==='balance-risk'&&!saved[a.id]);if(!risk)return;const box=document.getElementById('financeBox');if(!box)return;const el=document.createElement('div');el.id='homeBalanceRisk';el.innerHTML=`<details class="home-balance-risk"><summary title="余额预计不足2天">余额偏低</summary><div class="card"><p>${E(risk.desc)}</p>${button('查看资金',"App.go('billing')")}${button('忽略',`App.ignoreHomeIssue(${JSON.stringify(risk.id)})`,'link-btn')}</div></details>`;(box.querySelector(':scope > div')||box).append(el);};
 const afterHome=A.after_dash;A.after_dash=function(){afterHome?.call(this);this.syncHomeBalanceRisk();};
 ...
 A.view_matureDash=function(){
  const plans=DB.campaigns.filter(c=>c.status!=='archived'),date=new Date().toISOString().slice(0,10),until=new Date(Date.now()+7*864e5).toISOString().slice(0,10),count=k=>plans.filter(c=>this.homePlanState(c)===k).length;
  const states=[['ready','可投放'],['review','待审核'],['upcoming','未开始'],['blocked','暂不可投'],['unknown','待确认']];
  const schedule=plans.flatMap(c=>[{c,date:c.start,label:'开始'},{c,date:c.end,label:'结束'}]).filter(x=>x.date>=date&&x.date<=until).sort((a,b)=>a.date.localeCompare(b.date));
  return `<div class="advertiser-home home-four-home"><div class="home-heading"><h1>账户概览</h1></div>${this.renderHomeIssues()}<div class="home-sections-stack"><section class="card home-overview-side"><div class="home-section-title"><h2>投放概况</h2>${button('管理计划',"App.go('plans')")}</div><div class="home-side-states">${states.map(([k,label])=>`<button class="home-compact-state" onclick="App.openHomePlans('${k}')"><span>${label}</span><strong>${count(k)}</strong></button>`).join('')}</div><div class="home-secondary-states">${button('已暂停 '+count('paused'),"App.openHomePlans('paused')",'link-btn')}${button('已结束 '+count('ended'),"App.openHomePlans('ended')",'link-btn')}<span>${date} · UTC · 可投放不代表已产生曝光</span>${count('unknown')?'<span>部分状态更新延迟，不代表停投</span>':''}</div></section><section class="card home-overview-side"><div class="home-section-title"><div><h2>近期排期</h2><p>未来7天 · 共${schedule.length}项${schedule.length>3?'，显示最近3项':''}</p></div>${button('查看计划',"App.go('plans')")}</div>${this.renderHomeSchedule(schedule.map(x=>({...x,label:x.label==='开始'?'开始投放':'排期结束'})))}</section></div><nav class="home-quicklinks" aria-label="常用操作"><span class="cell-sub">常用入口</span>${button('数据报表',"App.go('report')",'link-btn')}${this.canOpen('creatives')?button('广告素材',"App.go('creatives')",'link-btn'):''}${this.canOpen('billing')?button('资金记录',"App.go('billing')",'link-btn'):''}${button('账户设置',"App.go('settings')",'link-btn')}${button('帮助中心',"App.go('help')",'link-btn')}</nav></div>`;
