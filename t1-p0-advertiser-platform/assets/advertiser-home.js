...
(()=>{
 ...
 wrap('renderUnifiedPlans',function(old){old();if(!this.homeListState)return;const state=this.homeListState;this.homeListState=null;
  ...
  this.modal(`<div class="modal-head"><div><h3>${label}的计划</h3><p>${plans.length}个计划</p></div>${action('关闭','App.closeModal()')}</div><div class="modal-body">${plans.map(c=>`<div class="home-plan-row"><div><b>${esc(c.alias||c.name)}</b><p>${esc(c.id)} · ${esc(c.mode.toUpperCase())}</p></div>${action('查看计划',`App.closeModal();${openPlan(c)}`)}</div>`).join('')||'<p>暂无符合条件的计划</p>'}</div>`,true);
 });
 A.view_matureDash=function(){
  ...
  return `<div class="advertiser-home"><div class="home-heading"><div><div class="home-eyebrow">账户概览</div><h1>掌握投放，开始今天的工作</h1><p>查看当前投放情况，安排接下来的推广。</p></div><div class="home-actions">${action('联系运营','App.showCpdContact()')}${this.role().mutate?action('新建 RTB 投放','App.startRtbCreate()','btn btn-primary'):''}</div></div>
  ${alerts.length?`<section class="home-priority"><div class="home-section-title"><h2>需要优先处理</h2><span>${alerts.length}项</span></div>${alerts.slice(0,3).map(a=>`<div class="home-alert"><div><b>${esc(a.title)}</b><p>${esc(a.desc)}</p></div>${action(a.label,a.call,'btn btn-primary btn-sm')}</div>`).join('')}${alerts.length>3?action('查看全部问题','App.showHomeAlerts()'):''}</section>`:''}
  <section class="home-status card"><div class="home-section-title"><div><h2>投放概况</h2><p>当前计划状态 · ${date}（UTC）</p></div>${action('管理全部计划',"App.go('plans')")}</div><div class="home-status-grid">${states.map(([key,label,color])=>`<button class="home-stat" onclick="App.openHomePlans('${key}')"><span class="home-stat-label"><i class="home-dot ${color}"></i>${label}</span><strong>${count(key)}</strong><span class="home-stat-link">查看计划 →</span></button>`).join('')}</div><div class="home-status-foot">${plans.length} 个未归档计划 · 可投放不代表已产生曝光</div></section>
  <div class="home-columns"><section class="card home-schedule"><div class="home-section-title"><div><h2>近期排期</h2><p>未来7天的开始与结束安排</p></div></div>${schedule.length?schedule.slice(0,5).map(s=>`<div class="home-schedule-row"><div class="home-date"><b>${s.date.slice(8)}</b><span>${s.date.slice(5,7)}月</span></div><div class="home-schedule-name"><b>${esc(s.c.alias||s.c.name)}</b><p>${esc(s.c.mode.toUpperCase())} · ${s.label}${s.c.status==='paused'?' · 当前已暂停':''}</p></div>${action('查看',openPlan(s.c))}</div>`).join(''):`<div class="home-empty"><span>◷</span><b>近期没有排期变更</b><p>未来7天没有计划开始或结束。你可以到投放管理查看完整排期。</p>${action('查看投放计划',"App.go('plans')")}</div>`}</section>
 ...
 A.showHomeAlerts=function(){this.modal(`<div class="modal-head"><h3>需要优先处理</h3>${action('关闭','App.closeModal()')}</div><div class="modal-body">${this.homeAlerts().map(a=>`<div class="home-alert"><div><b>${esc(a.title)}</b><p>${esc(a.desc)}</p></div>${action(a.label,`App.closeModal();${a.call}`)}</div>`).join('')}</div>`,true);};
